...
plt.tight_layout()
plt.subplots_adjust(hspace=0.3, wspace=0.2)  # Adjusted spacing
fig.subplots_adjust(left=0.04, right=0.98, top=0.96, bottom=0.07)
...
ax[0, 0].plot(np.arange(0, num_epochs*len_epoch, len_epoch), ensemble_loss_epoch, label=('ensemble ' if bool_plot_ensemble else '')+'loss (per epoch)', alpha=0.95)
...
ax[0, 0].grid()
ax[0, 0].legend()
ax[0, 0].set_title(f'Training Loss, {model_name}' if not bool_plot_ensemble else f'Ensemble Losses ({"+".join(model_list)})')
...
ax[0, 1].grid()
ax[0, 1].legend()
ax[0, 1].set_title(f'Validation MRR, {"ensemble" if bool_plot_ensemble else model_name}')
